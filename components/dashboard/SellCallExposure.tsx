...
export default function SellCallExposure({ options, timeRange = 'all' }: SellCallExposureProps) {
  ...
  const top5Items = useMemo(() => {
    ...
    const mapped = filteredOptions.map(o => {
      ...
      const covering_shares = o.net_contracts * sharesPerContract;
      ...
      return {
        ...o,
        strikePrice,
        coveringShares: covering_shares,
        daysLeft,
        optionName
      };
    });
            ...
            <tr className="h-11">
              ...
              >
                <div className="flex items-center gap-1 justify-end">
                  <span>{t('exposure.covering_shares')}</span>
                  {sortField === 'exposure' && (
                    sortOrder === 'asc' ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />
                  )}
                </div>
          ...
          <tbody>
            {topSellCalls.length > 0 ? (
              topSellCalls.map((option) => (
                <tr 
                  key={option.id}
                  className="h-11 border-b border-border/50 hover:bg-muted/50 transition-colors cursor-pointer group"
                ...
                >
                  ...
                  <td className="text-right px-4 font-medium text-[#F96E5B]">
